...
- Create base masks to used for Illumina basecalling with bcl2Fastq versions > 2.1.7.
- Create summary tables and plots from Illumina basecalling with bcl2Fastq versions > 2.1.7.
...
To update a sample sheet, first the parameters for the Run must be captured using the **setBCL2FastQparams** command. The resulting  BCL2FastQparams object contains information on the Run parsed from the runParameters.xml such as the actual index length which may be required to correct the sample sheet. 
...
In this first example we pass the **validateBCLSheet** function a sample sheets for versions < 1.8.9 and update it to be compatable with version > 2.1.7
...
The sample sheet from versions <= 1.8.9 contains many header which will need to be updated to those used versions >= 2.1.7 as well as superflous header which are maintained as metadata.
...
The basecallQC package can also provide the base masks and basecalling command for bcl2Fastq2 versions > 2.1.7 from the cleaned sample sheet and the BCL2FastQparams object.
...
Following the creation of a base masks tibble data.frame, the **createBCLcommand** function takes the cleaned sample sheet, a BCL2FastQparams object and the base masks data.frame to create the command to be used for Illumina basecalling/demultiplexing using  bcl2Fastq2 versions > 2.1.7.
