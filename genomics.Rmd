...
subFoldersFull <- subFoldersFull[!basename(subFoldersFull) %in% runsToExclude]
    ...
    for(l in uniqueIndexTypes){
      ...
      if(!file.exists(gsub("\\.csv",paste0("_",l,"\\.sh"),sampleSheetName))){
        ...
        if(as.numeric(as.vector(currentRunParameters$IndexRead1)) != 0){
          ...
          if(indexLengths[1] == 0){ indexLengths[1] <- ""}
        ...
        if(as.numeric(as.vector(currentRunParameters$IndexRead2)) != 0){
          ...
          if(indexLengths[2] == 0){ indexLengths[2] <- ""}
      ...
      shellBCLs <- c(shellBCLs,gsub("\\.csv",paste0("_",l,"\\.sh"),sampleSheetName))
    }
    }
  }else{
